...
async function updaddWord(req, res, next) {
  ...
  console.log(
    'owner, category, word, newWord, timestamp:',
    owner,
    category,
    word,
    newWord,
  ...
  try {
    ...
    if (word === newWord) {
      console.log('word equals newWord so this is an add operation.');
      ...
      if (findWord) {
        if (findWord['deleted'] === true) {
          ...
          console.log(
            'updadd-word: word equals new word, word was found, and deleted flag is false: No work to do.'
          );
      ...
      if (findWord) {
        ...
        if (findNewWord) {
          ...
          // but newWord does NOT exist so just update word
